...
t = 0;
for i=1:length(run)
    [n(i), k(i), sn(i), sk(i), th(i), sth(i)] = fits(run{i}, index, wl(i));
end
%% Summary
figure
...
title('Y_2O_3 Sample 1, k');
xlabel('\lambda, nm');
ylabel('k');
legend('fit','CXRO');
figure
errorbar(wl, th, sth, 'r.');
title('Y_2O_3 Sample 1 \sigma ');
xlabel('\lambda, nm');
ylabel('\sigma, nm');
gth=th(th>0 & th<2 & sth>0); % good thicknesses
gsth=sth(th>0 & th<2 & sth>0); % good standard deviations;
mth=sum(gth./gsth)/sum(1./gsth); % weighted mean
fprintf('Mean roughness is %f\n',mth);
