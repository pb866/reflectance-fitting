...
t = 0;
for i=1:length(run)
    [n(i), k(i), sn(i), sk(i), th(i), sth(i)] = fitt(run{i}, index, wl(i));
end
%% Summary
figure
...
gth=th(th>1); % good thicknesses
gsth=sth(th>1); % good standard deviations;
mth=sum(gth./gsth)/sum(1./gsth); % weighted mean
fprintf('Mean thickness is %f\n',mth);
title('Y_2O_3 Sample 1 SiO_2 Thickness');
xlabel('\lambda, nm');
ylabel('thickness, nm');
