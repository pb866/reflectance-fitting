function [n, k, sn, sk, th, sth] = fits(run, index, lambda)
...
b0 = [real(y2o3Index), imag(y2o3Index) 0.7];
...
[b, r, J, COVB, mse] = nlinfit(angle, refl, ...
    @(x,angle)fitfuncs(x,angle,lambda,siIndex,sio2Index), b0, opts, ...
     'ErrorModel', 'Combined');
% figure
% get rid of negative data
refl = refl(refl>0);
angle = angle(refl>0);
yf = fitfuncs(b,angle,lambda, siIndex, sio2Index);
...
saveas(gcf, ['figures/rs' num2str(round(lambda,1)) '.png']);
