function [n, k, sn, sk, th, sth] = fitt(run, index, lambda)
...
b0 = [real(y2o3Index), imag(y2o3Index) 1.8];
...
[b, r, J, COVB, mse] = nlinfit(angle, refl, ...
    @(x,angle)fitfunct(x,angle,lambda,siIndex,sio2Index), b0, opts, ...
     'ErrorModel', 'Combined');
% figure
% get rid of negative data
refl = refl(refl>0);
angle = angle(refl>0);
yf = fitfunct(b,angle,lambda, siIndex, sio2Index);
...
saveas(gcf, ['figures/rt' num2str(round(lambda,1)) '.png']);
