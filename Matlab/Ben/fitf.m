function [n, k, sn, sk] = fitf(run, index, lambda)
...
[b, r, J, COVB, mse] = nlinfit(angle, refl, ...
    @(x,angle)fitfuncf(x,angle,lambda,siIndex,sio2Index), b0, opts, ...
     'ErrorModel', 'Combined');
% figure
% get rid of negative data
refl = refl(refl>0);
angle = angle(refl>0);
yf = fitfuncf(b,angle,lambda, siIndex, sio2Index);
...
saveas(gcf, ['figures/rf' num2str(round(lambda,1)) '.png']);
